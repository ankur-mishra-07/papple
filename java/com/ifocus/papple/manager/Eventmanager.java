 ...
 */
public class Eventmanager implements Callback<SignupResult> {
     ...
     */
    public Eventmanager(Activity activity, int serviceCode,
    ...
    public void callPostRestService() {
        switch (serviceCode) {
            ...
            case Constant.EVENT_CREATION:
                //using Retrofit for making web service call
                EventCreator eventType = new EventCreator(
                        mapData.get(activity.getString(R.string.user_id)),
                        mapData.get(activity.getString(R.string.event_topic)),
                        mapData.get(activity.getString(R.string.event_desc)),
                        mapData.get(activity.getString(R.string.event_category)),
                        mapData.get(activity.getString(R.string.post_type)),
                        mapData.get(activity.getString(R.string.post_status))
                        );
                // prepare call in Retrofit 2.0
                iUserAuthApi = retrofit.create(IUserAuthApi.class);
                Call<SignupResult> call = iUserAuthApi.crateEvent(eventType);
                call.enqueue(this);
                break;
